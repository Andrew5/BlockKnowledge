...
- (void)test8 {
    ...
    /*
     1. block 捕获变量存在结构体里
     2. 访问结构体需要多条指令
     3. 如果能绕过结构体，直接一条指令成功，这个问题就没有了，比如 mov %rax, -20(%rbx)
     为啥__block修饰的变量要生成一个新的结构体来存这个变量，不能直接像捕获局部static变量那样？
     block本身也会对应一个imp结构体，直接存在这个结构体里面和新建一个结构体，然后通过__forwarding指针来访问感觉区别不大啊[捂脸]
     //        int c = sizeof(int);
     //        printf("%d",c);
     //        __block int count1;
     ...
     //            while (YES) {
     //                NSLog(@"read---%@--%p",[obj description],obj);
     //                NSLog(@"read次数---%i",count1++);
     //            }
     //        });
     //        while (YES) {
     //            obj = [NSObject new];
     //            NSLog(@"write---%@--%p",[obj description],obj);
     //            NSLog(@"write次数---%i",count2++);
     //        }
     //        void (^blk)(void) = ^{
     //            printf("Block\n");
     //        };
